...
namespace ConsoleApplication2
{
    class Program
    {
        ...
        private static void listdir(string[] Args)
        {
            ...
            AIConsole.Write(" COS/base/users/UNKNOWN/home", ConsoleColor.Blue,
        ...
        {
            AIConsole.Write(" Listing contents of: ", ConsoleColor.White,
			    new Random().Next(500, 1000));
            AIConsole.Write("COS/base/users/UNKNOWN/home\n", ConsoleColor.Blue,
			    new Random().Next(500, 1000));
            AIConsole.Write("     config.txt           " +
			    "Documents            " +
			    "TotallyNotAMeme.PNG     \n" +
			    "     RUN_INIT.txt    ",
			    ConsoleColor.White, new Random().Next(500, 1000));
        ...
        private static void Music(string[] Args)
        {
            ...
            if (Args != null)
            {
                ...
                {
                    AIConsole.Write(" Please enter a valid song name, " +
				    "it has to be exact to the name in the list " +
				    "above\n\n");
                }
            }
            else
            {
                AIConsole.Write(" Please enter a valid song name, it has to be " +
				"exact to the name in the list above\n\n");
                AIConsole.Write(" Type smusic followed by a song name to play " +
				"the song\n\n");
            }
	...
	private static void Read(string[] Args)
        ...
        static void Main(string[] args)
        {
            ...
            CommandProcessor.AddCommand("Swrite", " Simple document writer",
					write, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Smusic", " Simple music player",
					Music, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Scalc", " Simple calculator",
					calculator, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Exit", " Exits the console",
					Exit, CommandFlags.DEFAULT);
            //CommandProcessor.AddCommand("Test", "Example Command", ExampleCommand, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Credits", "Shows the people who made this",
					Credits, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("init", " <command description not found>",
					init, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Read",
					" Prints the contents of a text file to " +
					"the screen", Read, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("ListDir",
					" List the current working directory",
					listdir, CommandFlags.DEFAULT);
            //CommandProcessor.AddCommand("TestArgs", "Example Command For Args", ExampleCommandArgs, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Help", " Shows Commands", Help,
					CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("List",
					" List the contents of the current directory",
					list, CommandFlags.DEFAULT);
            CommandProcessor.AddCommand("Clear"," Clears the screen", clear,
					CommandFlags.DEFAULT);
            ...
            AIConsole.Write(" Type help on the next line to see a list of " +
			    "commands:\n\n",
			    ConsoleColor.White, 300, 500);
